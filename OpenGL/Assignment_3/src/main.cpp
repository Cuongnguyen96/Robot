...
void spawnExplosion(const cVector3d& pos, const cColorf& color = cColorf(1.0f, 0.4f, 0.1f, 1.0f)) {
    Explosion exp;
    exp.mesh = new cShapeSphere(exp.currentRadius);
    exp.mesh->setLocalPos(pos);
    
    // Transparent expanding ring with custom color
    safeSetColor(exp.mesh, color);
...
void spawnEnemy() {
    ...
    std::cout << "[DEBUG_SPAWN] Creating OBB collision detectors..." << std::endl; std::cout.flush();
    if (e->mesh) {
        for (unsigned int i = 0; i < e->mesh->getNumMeshes(); ++i) {
            cMesh* subMesh = e->mesh->getMesh(i);
            if (subMesh) {
                cCollisionOBB* col = new cCollisionOBB();
                col->initialize(subMesh);
                subMesh->setCollisionDetector(col);
            }
        }
    }
...
bool createTowerAt(const cVector3d& pos) {
    ...
    // Build OBB tree for physics proxy detection
    if (t->baseMesh) {
        cCollisionOBB* col = new cCollisionOBB();
        col->initialize(t->baseMesh);
        t->baseMesh->setCollisionDetector(col);
    }
...
void mouseButtonCallback(GLFWwindow* window, int button, int action, int mods) {
    if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) {
        ...
        if (playerGold >= TOWER_COST && !gameOver && !gameWin) {
            ...
            std::cout << "[MOUSE] Left Click! Requesting tower placement at " << sharedCursorPos << std::endl; std::cout.flush();
        }
    }
    else if (button == GLFW_MOUSE_BUTTON_RIGHT && action == GLFW_PRESS) {
        std::lock_guard<std::recursive_mutex> lock(game_mutex);
        if (playerGold >= 15 && !gameOver && !gameWin) {
            // Find nearest tower within 1.0 meters tolerance
            Tower* targetTower = nullptr;
            double nearestDist = 1.0;
            for (auto* t : activeTowers) {
                double d = cDistance(sharedCursorPos, t->position);
                if (d < nearestDist) {
                    nearestDist = d;
                    targetTower = t;
                }
            ...
            if (targetTower) {
                if (targetTower->health < targetTower->maxHealth) {
                    playerGold -= 15;
                    targetTower->health = std::min(targetTower->maxHealth, targetTower->health + 35.0);
                    // Spawn green celebratory repair effect
                    spawnExplosion(targetTower->position, cColorf(0.2f, 1.0f, 0.2f, 1.0f));
                    std::cout << "[REPAIR] Repaired Tower at " << targetTower->position << "! Recovered +35 HP, Current HP: " << targetTower->health << ". Gold remaining: " << playerGold << std::endl; std::cout.flush();
                } else {
                    std::cout << "[REPAIR] Tower at " << targetTower->position << " is already at maximum HP (" << targetTower->maxHealth << ")." << std::endl; std::cout.flush();
                }
            }
        }
    }
...
void initGame() {
    ...
    if (fontLoaded) {
        ...
        instructionsLabel->setText("Controls: Left Click - Place Tower (50g) | Right Click - Repair (15g, +35 HP)\nKeyboard: [Esc] - Quit | Auto-Progression Campaign Active");
